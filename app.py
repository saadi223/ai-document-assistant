...
Be clear and concise. Reply in the language of the question where practical.'''
    ...
    # Normalize common citation styles produced by LLMs.
    # Examples: 【S1】, (S1), [ S1 ] -> [S1]
    answer = re.sub(r'【\s*S(\d+)\s*】', r'[S\1]', answer, flags=re.IGNORECASE)
    answer = re.sub(r'\(\s*S(\d+)\s*\)', r'[S\1]', answer, flags=re.IGNORECASE)
    answer = re.sub(r'\[\s*S\s*(\d+)\s*\]', r'[S\1]', answer, flags=re.IGNORECASE)
...
def main():
    ...
    if ready and result and result['question'] == question.strip():
        ...
        # Accept normalized [S1] citations and full-width 【S1】 as a fallback.
        citation_matches = re.findall(r'\[S(\d+)\]|【S(\d+)】', result['answer'], flags=re.IGNORECASE)
        cited = {int(a or b) for a, b in citation_matches if a or b}
