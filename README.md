...
Upload PDF, DOCX, TXT or Markdown files, or import accessible Google Drive documents. Passage size and overlap are selected automatically for each document.
This is a learning project, not a guarantee of accurate answers or a production multi-user document platform.

## Update: automatic settings and Google Drive import

### Update your existing app

1. Extract the updated ZIP.
2. Replace **app.py**, **requirements.txt**, and **README.md** in the same GitHub repository. Keep these exact filenames.
3. Commit the changes. Streamlit should rebuild after the dependency change; reboot the app if necessary.
4. Keep your existing Groq secrets. No additional API key is required for public Drive link imports.
5. On a local installation, rerun `python -m pip install -r requirements.txt`, then restart Streamlit.

### Use automatic settings

Upload or import a document, then click **Process Documents**. The app selects settings separately for each document. Expand **Automatically selected settings** to inspect the target tokens, overlap, average characters and chunk count. A token is a model word piece, not one character. Short documents may create only one chunk and need no overlap. You do not need to choose any numbers.

Selection uses text layout plus the embedding model tokenizer; an LLM does not review the whole document to optimize settings. A measured “best” configuration would require a representative question/answer evaluation set.

### Import from Google Drive

1. In Drive, open the individual document and use **Share → Copy link**.
2. The app can only download files that are accessible without signing in and allow downloads. For non-sensitive files you are authorized to share publicly, you may choose **Anyone with the link → Viewer**. Do not change confidential files to public just to use this feature; download them yourself and use the regular uploader instead.
3. Paste the link in **Google Drive link**.
4. Click **Import / refresh link** and wait for the imported filename.
5. Click **Process Documents**, then ask your question.
6. To fetch later edits to the same Drive document, paste its link and click **Import / refresh link** again. An unchanged file reuses its index; changed bytes require processing again.
7. Use **Remove** beside an imported document, or **Clear documents and results** to remove everything.

Supported sharing links:

- `https://drive.google.com/file/d/FILE_ID/view` for PDF, DOCX, TXT and MD.
- `https://drive.google.com/open?id=FILE_ID` and download-style links for the same files.
- `https://docs.google.com/document/d/FILE_ID/edit` exported as DOCX.
- `https://docs.google.com/presentation/d/FILE_ID/edit` exported as PDF.
- `https://docs.google.com/spreadsheets/d/FILE_ID/edit` exported as PDF, using Google's default print/export layout.

Native Docs have no page numbers in DOCX extraction. Slides/Sheets citations refer to exported PDF pages. Sheets import is text retrieval from its PDF export, not spreadsheet calculation or complete cell/formula analysis. Complex export layouts can omit or reorder information. For native documents shared through a generic Drive link, open the actual document and copy its Docs/Sheets/Slides URL if the generic link fails.

Not supported: Drive folder imports, private/account-restricted files, disabled downloads, arbitrary website links, and other file types. Native Google document exports may be blocked by the owner's permissions or Google's quota. Public download routes are best-effort and may change. Failed import does not overwrite a previously imported document; remove the old file if you no longer want it included.

The total upload/import allowance is 25 MB. Redirects are restricted to Google document-download hosts, and responses have size/time limits. Login/error HTML is rejected rather than indexed as document content. Some Google confirmation pages may require manual download. Drive URLs are not sent to Groq; retrieved document excerpts are.
...
2. **Automatic chunking:** inspect each document’s text structure and use the embedding model’s tokenizer to choose a target of 160, 192 or 224 tokens, capped by the model’s input capacity. Overlap is approximately 12%, 17% or 22%. Prefer sentence/paragraph endings where possible. Verify each actual passage fits the model before embedding. PDF passages stay within their original page. These are adaptive heuristic defaults, not measured optimal settings; no Groq call is needed to choose them.
...
- Change, import, remove or refresh a document: if the content changes, questions should be disabled until reprocessing.
...
2. `automatic_chunks` (the old character-based `make_chunks` remains as a reference function)
...
There are no chunk sliders. The `automatic_chunks` function selects per-document settings. Inspect them in the Automatically selected settings expander. The embedding model is `EMBED_MODEL`; the default answer model is `DEFAULT_GROQ_MODEL`. The secrets setting overrides the default answer model.
...
- Automatic chunking verifies each passage against the embedding tokenizer limit. This avoids over-length document chunks; the model’s language limitations still apply. No cross-page overlap is created.
...
| Wrong answer | Inspect retrieved sources or rephrase the question; do not assume citations prove correctness |
...
Not completed: downloading/running the actual Sentence Transformers embedding model (its tokenizer was downloaded and tested in this update), live Groq calls, end-to-end answer accuracy, or a Cloud deployment. Those require validation in your environment. No API key is included.

Update checks completed: actual MiniLM tokenizer on English, Urdu and technical samples (all chunks within the 256-token embedding limit); adaptive chunk/page metadata checks; mocked HTTP tests for Drive formats, resource keys, redirects, confirmation, permission/error HTML, size limits and disallowed hosts; Streamlit initial render with no sliders, Drive input and invalid-link feedback. Real Drive download, embedding inference and live Groq calls remain untested.
...
- Google download/export permissions: https://developers.google.com/workspace/drive/api/guides/manage-downloads
